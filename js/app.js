...
function init() {
  ...
  if(localStorage.getItem('masjid') == null) {
    ...
    localStorage.setItem('teks', 'Tekan tombol Backquote ( ` ) untuk mengubah konfigurasi standar');
  ...
  fetch(BASE_URL + 'kota/semua')
    ...
    .then(kota     => {
      document.querySelector('#masjid').textContent = localStorage.getItem('masjid');
      document.querySelector('#alamat').textContent = localStorage.getItem('alamat');
      document.querySelector('#teks').textContent   = localStorage.getItem('teks');
...
function showModal(daftarKota) {
  document.addEventListener('keypress', (key) => {
    if(key.code == 'Backquote') {
      ...
      document.querySelector('#modal-masjid').value = localStorage.getItem('masjid');
      document.querySelector('#modal-alamat').value = localStorage.getItem('alamat');
      document.querySelector('#modal-teks').value   = localStorage.getItem('teks');
      document.querySelector('#modal-iqamah').value = localStorage.getItem('iqamah');
...
function showRealTime() {
  ...
  const hariArr  = ['Ahad', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jum\'at', 'Sabtu'];
  ...
  setInterval(() => {
    ...
    document.querySelector('#tanggal').textContent = date.getDate() +' '+ bulanArr[date.getMonth()] +' '+ date.getFullYear();

    // siapkan jam, menit dan detik
    let h = (date.getHours() < 10)    ? "0".concat(date.getHours())   : date.getHours();
    let i = (date.getMinutes() < 10)  ? "0".concat(date.getMinutes()) : date.getMinutes();
    let s = (date.getSeconds() < 10)  ? "0".concat(date.getSeconds()) : date.getSeconds();
    ...
    for(let i=0; i<wJadwal.length; i++) {
      if(his.substr(0, 5) == document.querySelector(wJadwal[i]).textContent) {
        ...
        else if(document.querySelector('#jam').textContent.substr(6, 2) == 10) {
  ...
  fetchPrayerTime(new Date().getFullYear(), new Date().getMonth(), new Date().getDate());
}


function fetchPrayerTime(tahun, bulan, tanggal) {
  fetch(BASE_URL + 'jadwal/' + localStorage.getItem('kode') + '/' + tahun + '/' + bulan + '/' + tanggal)
    .then(response    => response.json())
    .then(waktuSholat => {
...
function saveConfiguration() {
  ...
  localStorage.setItem('kode', document.querySelector('#modal-kabupaten').value);
